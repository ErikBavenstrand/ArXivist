...
from arxivist.application.ports.persistence.repository import CategoriesNotFoundError, PapersNotFoundError
from arxivist.application.ports.persistence.unit_of_work import AbstractPaperRepository, AbstractUnitOfWork
...
class FakePapersRepository(AbstractPaperRepository):
    ...
    def list_categories(self, limit: int | None) -> list[model.Category]:
        sorted_categories = sorted(self.categories, key=lambda x: str(x.identifier))
        if limit is None:
...
class FakeArXivCategoryExtractor(AbstractArXivCategoryExtractor):
    def __init__(
        self,
...
class FakeArXivPaperExtractor(AbstractArXivPaperExtractor):
...
class TestFetchAndStoreCategories:
    def test_fetch_categories_success(self) -> None:
        ...
        fetch_and_store_categories(fake_extractor, fake_uow)
        ...
        stored_categories = fake_uow.papers.list_categories(None)
...
class TestFetchAndStoreLatestPapers:
    def test_fetch_latest_papers_success(self) -> None:
        ...
        fake_extractor = FakeArXivPaperExtractor(fake_papers)
        fake_uow = FakeUnitOfWork()
        ...
        fetch_and_store_latest_papers(
            categories=["cs"],
            arxiv_paper_extractor=fake_extractor,
            uow=fake_uow,
        )

        stored_papers = fake_uow.papers.list_papers(None)
