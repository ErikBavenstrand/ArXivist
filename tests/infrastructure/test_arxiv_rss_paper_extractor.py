...
from arxivist.infrastructure.arxiv_rss_paper_extractor import ArXivRSSPaperExtractor, PaperDTO
from arxivist.infrastructure.exceptions import ArXivRSSMissingFieldError


class FakeRSSClient(AbstractRSSClient):
...
class TestArXivRSSPaperExtractor:
    def test_fetch_papers_success(self) -> None:
        fake_rss_fetcher = FakeRSSClient({
            "entries": [
                {
                    "id": "2025.12345",
                    "title": "Sample Paper",
        ...
        papers = client.fetch_recent_papers([model.Category(archive="cs", subcategory="CV")])
    ...
    def test_fetch_papers_missing_fields(self) -> None:
        fake_rss_fetcher = FakeRSSClient({"entries": [{}]})

        client = ArXivRSSPaperExtractor(fake_rss_fetcher)

        with pytest.raises(ArXivRSSMissingFieldError, match="Missing required field 'id' in the RSS feed entry"):
            client.fetch_recent_papers([model.Category(archive="cs", subcategory="CV")])
